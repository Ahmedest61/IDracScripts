      ...
      #  hp_list = []
       ...
       #     hp_dict = {}
            if node.idrac_ip == None:
                ...
                memory = self.extract_mem(filename)
                os.remove(filename)
 ...
 #       filename = self.ssh_idrac(ip, usr, pas)
  #      controller_key_list = list(controller_key_val_dict.keys())
   #     for key_ind in range(0, len(controller_key_val_dict)):
    ...
    def extract_idrac_model(self,fil):
        Config = self.extract_section(fil)
        for s in Config.sections():
            if "InstanceID: System.Embedded" in s:
    ...
    def extact_controller_info(self,ip, usr, pas):
      ...
      #  controller_dic = OrderedDict()
        ...
        driv_type, tot_driv_siz, driv_info = self.extract_hard_drives(filename)
        self.dictionary['drives_type'] = driv_type
        self.dictionary['total_drives_size'] = tot_driv_siz
        self.dictionary['drives_info'] = driv_info
        self.extract_idrac_model(filename)
...
#        list.extend(mslots,threads,cpu_info,nslots,nics_name_list,ports_slot,nic_info,driv_type,tot_driv_siz,driv_info)
    ...
    def compare_values(self,control_dict):
        control_keys = list(control_dict.keys())
        extrctd_idrac_keys = list(self.dictionary.keys())
        print control_keys, extrctd_idrac_keys
#        for ind in range(0, len(control_dict)):
            
    def verify_jetstream(self, filename, compute_node, ipmi_user, ipmi_pass):
        
        for node in compute_node:
            if node.idrac_ip == None:
                print "idrac_ip not found in node list"
                continue;
            else:
                ...
                for s in Config.sections():
                    if "Controller" in s:
                        controller_key_val = OrderedDict(Config.items(s))
                        self.extact_controller_info(node.idrac_ip ,ipmi_user, ipmi_pass)
                        self.compare_values(controller_key_val)
                  #      print self.dictionary;
                        exit(1)
                    elif "Compute" in s:
                        compute_section = OrderedDict(Config.items(s))

                    elif "Storage" in s:
                        storage_section = OrderedDict(Config.items(s))
                        
                    elif "SAH" in s:
                        sah_section = OrderedDict(Config.items(s))
